...
export default class ColumnRepositoryDatabase implements ColumnRepository {
	...
	async save(column: Column): Promise<number> {
		const [columnData] = await this.connection.query("insert into vuejs.column (id_board, name, has_estimative) values ($1, $2, $3) returning id_column", [column.idBoard, column.name, column.hasEstimative]);
	...
	async update(column: Column): Promise<void> {
		await this.connection.query("update vuejs.column set name = $1 where id_column = $2", [column.name, column.idColumn]);
	}
